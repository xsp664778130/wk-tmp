...
`jenkins.yaml` 完成。生产密钥只创建在 K3s Secret 中，不写入仓库。
...
当前只有公网 IP，因此 API/Bridge 先使用 HTTP/WS。绑定已备案域名后，应立即用
HTTPS/WSS 替换，并在 Sites 中同步更新 `SKILLPORT_BACKEND_URL`。
...
Git main 分支
  -> Jenkins（每 2 分钟检查代码）
  -> Java 测试/打包 + Web 构建验证
  -> /opt/skillport/releases/<构建号-提交号>
  -> current 软链接切换
  -> systemd 重启
...
Jenkins 负责 Java 服务端和 Bridge JAR 的全自动发布。Web 仍使用项目现有的 Sites 身份能力，流水线会验证 Web 构建，但不把它改成匿名的 Nginx 静态站点。
...
```text
SKILLPORT_BACKEND_URL=https://api.your-domain.com
SKILLPORT_GATEWAY_KEY=与服务器完全相同的网关密钥
```

随后发布 Web。网关密钥只存在于 Web 服务端运行环境和 Java 服务器，不能放进浏览器代码。
